...
**DFC Data Agent（大风车数据智能体）** 帮你用自然语言查大风车业务数据。
...
大风车有 **测试 / 预发 / 线上** 三套 MySQL 数据环境。数据智能体输入框左下角 **「环境」** 下拉可切换；选择会保存在浏览器本地，下次打开仍生效。
...
- 本地无同域 Cookie 时：侧栏「粘贴 Token 同步登录」，或配置 `DFC_API_DEV_SSO_TOKEN`（Mars **测试外网**登录后从 `_security_token` 复制）。同步成功或刷新页面后，`/api/auth/me` 会带回工作台 `queryLoginUserInfo`、SSO `findUserInfoByToken`、`nameAndPhone` 的原始响应（`dfcUser.raw`）以及合并后的 `dfcUser.data`；Agent 调 HTTP 时自动注入 `shopCode` / `groupCode`。客户手机号仍以问题里的号码为准。
...
- **团队问法模板**：全员共享口径（含内置模板）；点击星标可将问法放入个人固定分类「我的收藏」。需要出图时，可选用「图表」分类问法（会明确要求柱状图/折线图/饼图/漏斗图/K 线等常见图形）。
- **最近对话**：最近 20 条会话，点击继续该对话

侧栏 **历史会话** 列出你在数据智能体中的完整对话（写入应用库 `agent_threads`）。点「继续对话」会打开 `/agents?threadId=`，可在原上下文上追问，并还原分析步骤、确认 SQL 卡片、结果表和图表；「新对话」会换一个新的会话 ID。未配置 `APP_MYSQL_*` 时会话仍会暂存在 Redis/内存，重启后可能丢失。

## 结果操作

- **导出 CSV**：查询结果表格右上角
- **下载图表**：出图后可在图表右上角下载 PNG 或 SVG 到本地
- **主题**：侧栏底部可在「白天 / 黑夜」之间切换，选择会保存在浏览器本地；默认白天
- **最近对话**：右侧历史面板，点击可继续该会话

## 工具管理

侧栏 **工具管理** 可查看 Agent 当前能调用的全部工具（库表探查、接口路由、SQL、图表等）。管理员可修改展示名/说明/启停，或新增只读 HTTP 工具（测试环境仅 `*.dasouche.net`）。`propose_sql` / `execute_sql` 为核心工具，不可停用或删除。
