...
export function AgentEnvSwitcher({
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        <span className="shrink-0 text-muted-foreground">环境</span>
      ...
      {open ? (
        <div
          ...
          role="listbox"
          aria-label="数据环境"
          className="absolute bottom-[calc(100%+6px)] left-0 z-50 w-[248px] overflow-hidden rounded-xl border border-border bg-elevated shadow-[var(--shadow-lg)]"
        >
          <div className="border-b border-border px-3 py-2">
            <p className="text-[11px] font-medium text-foreground">数据环境</p>
            ...
            {profiles.map((profile) => (
              <button
                ...
                className={cn(
                  ...
                  value === profile.id ? "bg-brand/10" : "hover:bg-surface-hover",
                )}
              >
                <span
                  className={cn(
