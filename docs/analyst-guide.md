...
侧栏 **历史会话** 列出你在数据智能体中的完整对话（写入应用库 `agent_threads`）。点「继续对话」会打开 `/agents?threadId=`，可在原上下文上追问；「新对话」会换一个新的会话 ID。未配置 `APP_MYSQL_*` 时会话仍会暂存在 Redis/内存，重启后可能丢失。

## 结果操作

- **导出 CSV**：查询结果表格右上角
- **下载图表**：出图后可在图表右上角下载 PNG 或 SVG 到本地
- **主题**：侧栏底部可在「亮色 / 暗黑」之间切换，选择会保存在浏览器本地；默认亮色
